...
const HomePage = () => {
  ...
  const {user,projects, loadingProjects, generatingProjects, loadProjects, handleGenerate,handleDelete,
    ...
    bg-cover bg-center bg-no-repeat">
      ...
      <nav className="sticky top-0 z-10 flex items-center justify-between px-6 py-4">
        ...
        <div className='flex items-center gap-4 text-sm font-medium text-zinc-300'>
            <span>{user?.name}</span>
            <button onClick={logout} className='py-1.5 px-3 border border-white/20 text-white hover:bg-white/
            10 text-xs rounded-md cursor-pointer bg-transparent'>
            Sign out
            </button>
        </div>
      </nav>
              ...
              <div className='mt-12 w-full'>
                ...
                <div className='space-y-2 max-h-[80vh] overflow-y-auto pr-1'>
                    ...
                    onClick={()=>navigate(`/builder/${p._id}`)}>
                      ...
                      <div className='flex items-center gap-2'>
                          <button className='p-1.5 rounded-md text-zinc-200 hover:text-red-400
                          ...
                          <ArrowRightIcon size = {14} className='text-zinc-300 group-hover:text-white'/>
                      </div>
                    </div>
                    ))}
                </div>
